...
const FALLBACK: MetricPoint[] = [
  { phase: "nvt", timePs: 0, hydrationContacts: 1 },
  { phase: "nvt", timePs: 1.5, hydrationContacts: 2 },
  { phase: "nvt", timePs: 3.0, hydrationContacts: 3 },
  { phase: "nvt", timePs: 4.5, hydrationContacts: 2 },
  { phase: "nvt", timePs: 6.0, hydrationContacts: 3 },
];

export function HydrationTracePlot({ progress, accentColor }: { progress: number; accentColor: string; rdfActiveRadius?: number }) {
      ...
      .then((next) => {
        ...
        const parsed = next.trajectory
          ...
          .map((entry: MetricPoint) => ({
            phase: entry.phase,
            timePs: entry.timePs,
            hydrationContacts: entry.hydrationContacts,
          }));
  ...
  const yMax = Math.max(2, ...data.map((point) => point.hydrationContacts)) + 0.8;
  const visibleTime = xMax * (0.24 + progress * 0.72);
  const visible = data.filter((point) => point.timePs <= visibleTime);
  const chartPoints = visible.length >= 2 ? visible : data.slice(0, 2);

  return (
    <PlotContainer ariaLabel="Hydration contact count around the focus caffeine from the NVT production trajectory">
      {({ height, innerWidth, innerHeight, margin, font }) => {
        ...
        const linePath = line<MetricPoint>()
          .x((d) => xScale(d.timePs))
          .y((d) => yScale(d.hydrationContacts))
          .curve(curveMonotoneX)(chartPoints) || "";
        const areaPath = d3Area<MetricPoint>()
          .x((d) => xScale(d.timePs))
          .y0(innerHeight)
          .y1((d) => yScale(d.hydrationContacts))
          .curve(curveMonotoneX)(chartPoints) || "";
        const visibleHighlights = highlights.filter((point) => point.timePs <= visibleTime);
        const valueByTime = new Map(data.map((point) => [point.timePs, point.hydrationContacts]));
