...
const EVIDENCE_DIR = path.join(ROOT, ".superloopy", "evidence", "allatom-visuals");
const VIEWPORTS = [
...
const SCENES = [
  { id: "15-allatom-branch", step: 0 },
  { id: "16-allatom-pbc", step: 1 },
  { id: "17-allatom-force-field", step: 2 },
  { id: "18-allatom-integrator", step: 3 },
  { id: "19-allatom-ensemble", step: 4 },
  { id: "20-allatom-observables", step: 5 },
  { id: "21-allatom-mapping", step: 6 },
];
  ...
  for (const viewport of VIEWPORTS) {
    ...
    for (const scene of SCENES) {
      await page.evaluate(({ step }) => {
        window.__multiscaleDebug?.jumpToScene("allatom", step, 0.5, null);
        window.scrollTo(0, 0);
      }, scene);
      await page.waitForSelector(".allatom-mechanism", {
        state: "visible",
        timeout: 30_000,
      });
      ...
      const metrics = await page.evaluate(() => {
        ...
        const surfaces = [...document.querySelectorAll('[data-testid="multiscale-render-surface"]')];
        const surface = surfaces[0] ?? null;
        const mechanism = document.querySelector(".allatom-mechanism");
      ...
      if (viewport.width < 768) {
        ...
        openCollision =
          openToolbar && intersects(openToolbar.title, openToolbar.toolbar);
      ...
      const interactions = {
        2: ["^결합 결합 항$", "^결합각", "^이면각", "^반데르발스", "^정전기"],
        5: ["^방향성$", "^분자 배치$", "^상호작용 모티프$"],
      }[scene.step];
      ...
      if (interactions) {
        for (const label of interactions) {
          ...
          const overflow = await page.evaluate(() => {
            ...
            return [...mechanism.querySelectorAll("*")]
              .filter(visible)
              .filter(
                (element) =>
                  element.scrollWidth > element.clientWidth + 1 ||
              ...
              .map((element) => element.className?.toString().slice(0, 100) ?? element.tagName);
          });
