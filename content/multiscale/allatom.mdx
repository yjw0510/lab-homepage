...
contentKo: |
  ...
  궤적의 여러 시점에서 같은 관측량을 계산하면 확산 계수와 이완 시간, 값의 분포를 얻는다. 전원자 MD는 액체 구조, 분자 이동, 고분자 질서화, 국소 역학 응답을 연구하는 데 적합하다. 결합 관계가 고정된 고전 역장은 구조와 수송을 다룬다. 결합이 바뀌는 반응은 반응형 역장이나 전자 계산으로 연구한다.
