...
A particularly striking case arises when linear chains are replaced with bottlebrush polymers, branched molecules with side chains extending from a central backbone. The asymmetry of the bottlebrush coating breaks the particle's spherical symmetry and opens access to two-dimensional assembly patterns that isotropic coatings cannot produce: hexagonal arrays, square lattices, and linear clusters.`,
    descriptionKo: `같은 금 나노입자를 같은 용매에 넣어도 전혀 다른 결정 구조로 자발 조립될 수 있다. 유일한 변수는 입자 표면의 고분자 코팅이다. 이 고분자 접합 나노입자는 반복 분자 단위로 이루어진 사슬을 달고 있으며, 이 사슬이 조절 가능한 껍질로서 이웃 입자와의 상호작용을 매개한다. 코팅의 성질(사슬 수, 길이, 분지 구조)을 조절하면 느슨한 체심입방 배열부터 조밀한 면심입방이나 육방 구조까지 조립의 결정 대칭이 바뀐다.

이것이 중요한 이유는 나노 규모의 결정 대칭이 빛과의 상호작용을 비롯한 벌크 물성을 직접 결정하기 때문이다. 분자 수준에서 고분자 껍질을 조절하는 것은 거시적 광학·역학 거동을 설계하는 손잡이가 된다.
...
블록 공중합체는 또 다른 방식의 자기조직화를 보여준다. 화학적으로 구별되는 두 개 이상의 분절이 끝끼리 이어진 이 사슬에서 분절들은 서로 밀어내기 때문에 교대 층, 원통 배열, 연결된 채널 네트워크 같은 주기적 나노구조로 자발 분리된다. 어떤 구조가 형성되는지는 블록의 상대적 길이와 조성에 달려 있다. 전산 방법으로 무질서에서 질서로의 전이 경로를 규명하며, 최종 구조뿐 아니라 준안정 중간체, 즉 실험에서 감지되지만 분자 수준 특성 규명은 어려운 평형 도달 경로상의 일시적 상태도 분해한다.`,
    icon: "Layers",
    color: "#8b5cf6",
    gridSpan: [1, 2],
...
형상이 제어하는 것은 색만이 아니다. 사족형 반도체 나노결정, 잭스 놀이 도구를 닮은 네 팔 입자는 구형 입자보다 훨씬 낮은 농도에서 전기 전도 네트워크를 형성한다. 팔이 더 적은 입자로 더 많은 이웃에 닿으므로 전자가 네트워크를 가로지르며 거쳐야 하는 호핑 단계가 줄어든다. 90 nm 팔을 가진 CdSe 사족형은 같은 부피 분율의 구형 나노결정 대비 7배 높은 전도도를 보여, 유연한 박막 전자 소자의 후보가 된다.
...
시간에 따른 탐침의 마찰 신호를 분석하면 두 가지 구별되는 완화 과정이 드러난다. 하나는 이웃 입자로 이루어진 국소 우리 안에서의 빠른 진동이고, 다른 하나는 주변 많은 입자가 관여하는 느린 협동적 재배열이다. 유리가 더 냉각될수록 이 협동적 영역의 크기가 커지며, 이는 유리의 서로 다른 영역이 극적으로 다른 속도로 완화된다는 동역학적 비균질성 가설의 직접적 증거이다. 벌크 실험으로는 간접적으로만 추론할 수 있는 현상이다.`,
    icon: "Atom",
    color: "#06b6d4",
    gridSpan: [1, 1],
    tags: ["Metallic Glass"],
...
폴리아크릴아마이드 겔(널리 사용되는 모형 시스템)의 조대화 시뮬레이션은 실험 측정 강성을 재현하며, 팽윤이 사슬 얽힘을 비선형적으로 줄인다는 점을 보인다. 물 함량을 두 배로 늘리면 강성은 절반보다 더 크게 떨어진다. 가교 전에 형태를 잡은 겔은 한 축으로 다른 축보다 더 단단한 방향 의존적 탄성을 보이며, 이는 평평한 겔에서는 나타나지 않고 생체 조직의 방향성 강성에 맞춰야 하는 조직 스캐폴드에 유용할 수 있다.

PLGA-PEG-PLGA 삼중블록 공중합체 기반 온도감응성 하이드로겔은 특히 우아한 설계 경로를 제공한다. 고분자는 실온에서 녹아 있다가 체온 근처에서 겔을 형성하여, 주사 시에는 액체이고 체내에서는 고체이다. 겔화가 날카롭게 일어나는지 서서히 일어나는지는 주쇄를 따라 구성 단위가 어떻게 배열되었는지에 달려 있다. 균일한 블록은 날카로운 전이와 단단한 겔을, 무작위 혼합은 점진적 전이와 약한 겔을 만든다.`,
    icon: "FlaskConical",
    color: "#10b981",
    gridSpan: [1, 1],
    tags: ["Hydrogel"],
